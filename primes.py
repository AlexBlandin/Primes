...
def millerprime(n):
  ...
  r, d = next((r, d) for r, d in enumerate(map(lambda r: (n - 1) // 2**r, range((n - 1).bit_length())))
              if d & 1 and n == 2**r * d + 1)
  assert (n == 2**r * d + 1)
...
def main():
  ...
  bigger_prime = 643808006803554439230129854961492699151386107534013432918073439524138264842370630061369715394739134090922937332590384720397133335969549256322620979036686633213903952966175107096769180017646161851573147596390153
